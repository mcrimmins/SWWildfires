...
ggplot(subset(fireClim, seq==0 & groupVeg %in% c("Conifer","Grassland","Shrubland") &
                seas %in% c("AMJ","JAS")),
       aes(groupVeg,spei3, fill=groupVeg))+
...
p1<- ggplot(subset(fireClim, seq==0 & groupVeg %in% c("Conifer","Grassland","Shrubland")),
       aes(tmeanZ, vpdmaxZ, color=as.factor(fireSizeClass2)))+
...
temp<-subset(fireClim,
             ...
             & fireSeas %in% c("JAS")
             & fireSizeClass2 %in% c("G+","F"))
temp<-temp[,c("groupVeg","seq","fireSeas","fireYear","fireSizeClass2","spi3","spei3","vpdmaxZ","tmeanZ")]
...
ggplot(data=subset(temp,var %in% c("spi3","vpdmaxZ")),
  ...
  ylim(-4,4)+
  ggtitle(paste0("composite antecedent seasonal climate - JAS"))
